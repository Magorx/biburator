...
class CoreMLStableDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        if False and safety_checker is None:
        ...
        **kwargs,
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            if callback is not None and i % callback_steps == 0:
                if callback(i, t, latents) is False:
                    break
...
def parse_args():
    ...
    parser.add_argument(
        "-i",
        # default= "models/coreml-stable-diffusion-2-base_split_einsum_packages",
        default="models/coreml-stable-diffusion-2-base_original_packages",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If true, the model will be mocked and the script will run without an actual model."
    )
...
def load_model(args):
    ...
    logger.info("Loading Core ML pipe")
    coreml_pipe = get_coreml_pipe(pytorch_pipe=pytorch_pipe,
                                  mlpackages_dir=args.i,
                                  model_version=args.model_version,
                                  compute_unit=args.compute_unit,
...
class ModelMock(CoreMLStableDiffusionPipeline):
